...
Últimas notas de la versión que describen las últimas funciones, mejoras y correcciones de [!DNL Adobe Scene7 Publishing System] 6.9: parte de la solución Adobe Experience Manager en el [!DNL Adobe Experience Cloud].
...
* **Diseño interactivo** - Optimice aún más la calidad de las experiencias de imagen adaptables definiendo diferentes ajustes preestablecidos o modificadores de imagen para cada punto de interrupción.
* **Personalización del visor** - Ahora puede establecer y modificar dinámicamente el estilo y los comportamientos del visualizador sobre la marcha.
* **Vista previa segura para Visual Configurator** : Obtenga una vista previa y pruebe el procesamiento visual de forma segura para experiencias personalizadas antes de insertarlas en la producción.
* **Capacidad de SSO (inicio de sesión único)** - Ahora puede iniciar Scene7 sin problemas desde Adobe Experience Cloud y utilizar su Adobe ID para iniciar sesión en Scene7.
...
* **Compatibilidad con capacidad de respuesta mejorada** - Optimice aún más la calidad de las experiencias de imagen adaptables definiendo diferentes ajustes preestablecidos o modificadores de imagen para cada punto de interrupción. Consulte [Uso de la biblioteca de imágenes estáticas adaptables](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/responsive-static-image-library/c-about-responsive-static-image-library.html).
...
* Los subtítulos de vídeo ya están disponibles de forma predeterminada `Universal_HTML5_MixedMedia_dark` y `Universal_HTML5_MixedMedia_light` espectadores. Consulte [Adición de subtítulos a vídeos](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/video/adding-captions-video.html#video).
...
* Prueba segura para solicitudes del servidor de procesamiento: la prueba segura para recursos de servicio de imágenes principales y recursos de vídeo ahora se amplía para incluir también viñetas. Consulte [Prueba de recursos antes de hacerlos públicos](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish).
* Monitorización de la función Invalidar caché de CDN. Después de seleccionar la plantilla de invalidación de CDN (consulte [Configuración general](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html#general-settings)) y un **[!UICONTROL Invalidar CDN]** Cuando se realiza una solicitud de, ahora aparece un indicador con una estimación de cuánto tiempo tardará en borrar la caché.

## Visores {#section-fc0577722d0b431dae3506c8d4e5366f}

**Nuevas funciones, mejoras y correcciones de errores para Scene7 HTML 5 Viewer SDK 2.8.1**

General

* Componentes que utilizan `insertBefore()` API ahora utilizar `appendChild()` API.
* Se ha añadido la compatibilidad con `setCSS()`, `setModifier()`, y `dispose()` métodos.
* Se ha añadido la compatibilidad con `getWidth()`, y `getHeight()` métodos.
* Se ha añadido la compatibilidad con `setPortrait()` y `setLandscape ()` métodos.
...
* Añadido `TableOfContents` componente.
...
* Se ha agregado compatibilidad para devolver sugerencias específicas de la orientación `MediaSetDesc` objetos.
...
* Fijo: la imagen se desplaza hacia la derecha si `DIV` tiene `text-align:center` estilo.
...
**Nuevas funciones, mejoras y correcciones de errores para los visualizadores de Dynamic Media Image Serving 5.1.1**
...
* Se han cambiado los visores a `Track requests` en lugar de `Page requests` para el seguimiento de Adobe Analytics.
...
* Se ha agregado compatibilidad para mostrar imágenes estrechas con `highlightmode` Modificador para el visor flotante de HTML5
* Se ha agregado compatibilidad para controlar la ampliación de imágenes mediante `zoomfactor` Modificador para el visor flotante de HTML5.
...
* El número total de recursos exportados resulta ser un bueno que el número original de los detalles del registro de trabajo de exportación.
