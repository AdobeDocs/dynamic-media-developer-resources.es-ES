...
Se puede usar una solicitud de servicio de imágenes completa como origen de capa especificándola en el comando `src=` (o `mask=`) con la siguiente sintaxis:

`…&src=is( nestedRequest)&…`

El token `is` distingue entre mayúsculas y minúsculas.

La solicitud anidada no debe incluir la ruta de acceso raíz del servidor (normalmente ` http:// *[!DNL server]*/is/image/'`).

>[!NOTE]
>
>Los caracteres delimitadores de solicitud anidados ( `'(',')'`) y los caracteres delimitadores de comando ( `'?'`, `'&'`, `'='`) dentro de las solicitudes anidadas no deben estar codificados en HTTP. En la práctica, las solicitudes anidadas deben codificarse igual que la solicitud externa (anidación).
...
>[!NOTE]
>
>Los caracteres delimitadores de solicitud anidados ( `'(',')'`) y los caracteres delimitadores de comando ( `'?'`, `'&'`, `'='`) dentro de las solicitudes anidadas no deben estar codificados en HTTP. En la práctica, las solicitudes incrustadas deben codificarse igual que la solicitud externa (incrustación).
...
Cuando el procesador de gráficos FXG (también conocido como [!DNL AGMServer]) está instalado y habilitado con el servicio de imágenes, las solicitudes FXG se pueden usar como fuentes de capa especificándolas en comandos `src=` (o `mask=`). Utilice la siguiente sintaxis:
...
>[!NOTE]
>
>Los caracteres delimitadores (`'(',')'`) y los caracteres delimitadores de comandos (`'?'`, `'&'`, `'='`) de las solicitudes anidadas no deben tener codificación HTTP. En la práctica, las solicitudes incrustadas deben codificarse igual que la solicitud externa (incrustación).
...
Para especificar una dirección URL externa para un comando `src=` o `mask=`, delimite la dirección URL externa o el fragmento de dirección URL con paréntesis:

`…&src=( foreignUrl)&…`

Importante: Los caracteres delimitadores (`'(',')'`) y los caracteres delimitadores de comandos (`'?'`, `'&'`, `'='`) de las solicitudes anidadas no deben tener codificación HTTP. En la práctica, las solicitudes incrustadas deben codificarse igual que la solicitud externa (incrustación).
...
El servidor almacena en caché las imágenes externas según los encabezados de almacenamiento en caché incluidos con la respuesta HTTP. Si no hay ni un encabezado de respuesta HTTP `ETag` ni Última modificación, la respuesta no se almacena en caché. Esto puede causar un mal rendimiento para los accesos repetidos para la misma imagen externa, ya que el servicio de imágenes necesita recuperar y revalidar la imagen en cada acceso.
...
[src=](../../../../../is-api/http-ref/image-serving-api-ref/c-http-protocol-reference/c-command-reference/r-src.md#reference-f6506637778c4c69bf106a7924a91ab1) , [máscara=](../../../../../is-api/http-ref/image-serving-api-ref/c-http-protocol-reference/c-command-reference/r-mask.md#reference-922254e027404fb890b850e2723ee06e), [Solicitud de preprocesamiento](../../../../../is-api/http-ref/image-serving-api-ref/c-http-protocol-reference/c-syntax-and-features/r-request-preprocessing.md#reference-c27976436bf04194bfbe9adf40ea98e3), Referencia de procesamiento de imágenes, [Plantillas](../../../../../is-api/http-ref/image-serving-api-ref/c-http-protocol-reference/c-templates/c-templates.md#concept-3cd2d2adae0e41b2979b9640244d4d3e), [Utilidades de servicio de imágenes](../../../../../is-api/is-utils/utilities/c-location-of-utilities.md#concept-bae61e53344449af978502cac6be8b5f)
