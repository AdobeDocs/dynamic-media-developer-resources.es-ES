...
Algunos comandos, incluido `extend=`, se aplican implícitamente a la capa 0 en lugar de a la capa compuesta cuando `layer=comp` se selecciona. En estos casos, todos los comandos asignados automáticamente a la capa 0 se aplicarán antes que los comandos que se apliquen a `layer=comp`. Por lo tanto, cuando `layer=comp`, `extend=` se aplica antes de `flip=`.
