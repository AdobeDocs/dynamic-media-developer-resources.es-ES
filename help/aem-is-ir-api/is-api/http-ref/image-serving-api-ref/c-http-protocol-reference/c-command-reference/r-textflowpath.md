...
Comandos de margen RTF `\margl`, `\margr`, `\margt`, y `\margb` se ignoran cuando `textFlowPath=` está presente. Si no se especifica ninguna definición de ruta, `textFlowPath=` se ignora.

## Propiedades {#section-b68dc887c6534ce8982cad740b3aeaa4}

Atributo de capa de texto ( `textPs=` solo). Ignorado por otras capas. Se aplica a `layer=0` si se especifica para `layer=comp`.
