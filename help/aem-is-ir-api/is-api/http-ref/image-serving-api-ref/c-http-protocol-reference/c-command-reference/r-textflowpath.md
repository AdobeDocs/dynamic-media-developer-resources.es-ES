...
Los comandos de margen `\margl`, `\margr`, `\margt`y `\margb` RTF se omiten cuando `textFlowPath=` están presentes. Si no se especifica ninguna definición de ruta, `textFlowPath=` se omite.

## Propiedades {#section-b68dc887c6534ce8982cad740b3aeaa4}

Atributo de capa de texto ( `textPs=` solo). Omitido por otras capas. Se aplica a `layer=0` si se especifica para `layer=comp`.
