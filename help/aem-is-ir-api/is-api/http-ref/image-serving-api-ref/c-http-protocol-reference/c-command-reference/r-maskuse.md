...
La siguiente tabla ilustra el efecto de `maskUse=` dependiendo de la disponibilidad y el tipo de máscara (canal alfa) asociada a la imagen de capa.
...
El comportamiento de `maskUse=` es indefinido y no se admite cuando se especifica con capas de texto o de color sólido cuando no se aplica ninguna máscara de imagen (especificada con `mask=` o `catalog::Mask`).
