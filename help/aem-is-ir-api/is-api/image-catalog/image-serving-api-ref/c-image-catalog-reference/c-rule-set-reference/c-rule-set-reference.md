...
Los elementos `<?xml>` y `<ruleset>` siempre son necesarios en un archivo XML de conjunto de reglas válido, aunque no se haya definido ninguna regla real.
...
Se proporciona una copia de [!DNL RuleSet.xsd] en la carpeta del catálogo y debe utilizarse para validar un archivo de conjunto de reglas antes de registrarlo en el [!DNL catalog.ini] archivo. Tenga en cuenta que el servicio de imágenes utiliza una copia interna de [!DNL RuleSet.xsd] para la validación.
...
El comportamiento predeterminado se puede modificar con el `OnMatch` atributo del `<rule>` elemento. `OnMatch` se puede establecer en `break` (predeterminado), `continue`o `error`.
...
[ [!DNL package java.util.regex] ](https://www2.cs.duke.edu/csed/java/jdk1.4.2/docs/api/) describe la implementación de expresión regular específica que utiliza el servicio de imágenes.
