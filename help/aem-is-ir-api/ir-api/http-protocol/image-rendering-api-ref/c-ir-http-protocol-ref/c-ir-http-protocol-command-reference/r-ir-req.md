...
A menos que se indique lo contrario en las descripciones detalladas, el servidor devuelve respuestas de texto con el tipo MIME &lt;text/plain>.
...
Devuelve las propiedades seleccionadas del archivo de viñeta o de la entrada de catálogo especificada en la ruta URL. Consulte [Properties](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-response-data/c-ir-properties.md#concept-e99f1a373eae4af9b41842ca0088ad3a) para obtener una descripción de la sintaxis de respuesta y el tipo MIME de respuesta. Se omiten otros comandos de la solicitud. Se devuelven las siguientes propiedades:
...
Los datos de respuesta constan de un elemento `<map>` que contiene un conjunto de elementos `<area>`, similares a la etiqueta HTML `<AREA>` .

Cada elemento `<area>` incluye los atributos estándar `type=` y `coord=`, así como un atributo `name=` que especifica el nombre del grupo de viñetas o la ruta del nombre. Si las máscaras del grupo de objetos correspondiente tienen regiones discontinuas, estarán presentes varios elementos `<area>` con el mismo nombre.
...
<table id="table_F3ECF0584F6247A2A75C1CAFE1C57A4E"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> <p> <span class="codeph"> seleccione  <span class="codeph"> selection.attributes  </span>ion.name  </span> </p> </td> 
   <td> <p> Cadena </p> </td> 
...
Devuelve el contenido de `vignette::UserData`. El servidor reemplazará todas las ocurrencias de `'??'` en `vignette::UserData` con caracteres de fin de línea ( `<cr><lf>`). La respuesta tiene el formato de datos de texto con el tipo MIME de respuesta definido como &lt;text/plain>.

Si el objeto especificado en la ruta URL no se resuelve en una entrada de mapa de viñetas válida o si `vignette::UserData` está vacío, la respuesta solo contendrá un terminador de líneas ( `CR/LF`).
