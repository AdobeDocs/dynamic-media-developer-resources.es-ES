...
<table id="simpletable_D04D41FBB03D4992B257CCBAD7EF0E7B"> 
 ...
 <tr class="strow"> 
  ...
  <td class="stentry"> <p>Devuelve el contenido de la <span class="codeph"> viñeta::UserData </span>. </p> </td> 
 </tr> 
</table>
...
Ejecuta los comandos especificados y devuelve la imagen procesada. Si se produce un error, se devuelve la información de error y depuración en lugar de la imagen de error ( `attribute::ErrorImagePath`).
...
<table id="table_A30296D29B5D43F1B5383A887252C6B4"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Cadena </p> </td> 
   <td colname="col3"> <p> <span class="codeph"> attribute::Modifier </span> o empty si no es una entrada de catálogo. </p> </td> 
  </tr> 
  <tr> 
...
Los datos del mapa no se escalan `wid=` ni se modifican `hei=` de otro modo. El tipo MIME de respuesta es `<text/xml>`.

Los datos de respuesta constan de un `<map>` elemento que contiene un conjunto de `<area>` elementos, similar a la `<AREA>` etiqueta HTML.

Cada `<area>` elemento incluye los atributos estándar `type=` y `coord=` , así como un `name=` atributo, especificando el nombre del grupo de viñetas o la ruta del nombre. Se mostrarán varios `<area>` elementos con el mismo nombre si las máscaras del grupo de objetos correspondiente tienen regiones discontinuas.

Además de los atributos predeterminados, las viñetas pueden definir atributos adicionales si se crean. Estos atributos personalizados se definen como atributos de grupo de objetos. Los nombres de los atributos personalizados deben comenzar por `map`. que se incluirán en los `<area>` elementos. Por ejemplo, si los atributos de grupo incluyen `map.href=http://www.scene7.com`, el `<area>` elemento correspondiente incluirá `href="http://www.scene7.com"`.

Se devuelve un documento XML con un `<map>` elemento vacío si la viñeta no incluye datos de asignación.

`object`

Ejecuta los comandos especificados y devuelve la imagen procesada enmascarada por la selección de objetos residuales (el grupo u objeto seleccionado con el último `sel=` o `obj=` comando de la solicitud). Normalmente se utiliza junto con un formato de imagen que admite alfa (consulte [fmt=](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-fmt.md#reference-4c743f67d56b47c5b774fcc900ff758c)). Si se utiliza un formato de imagen que no admite alfa, las áreas fuera de la máscara son de color negro.

`props`

Ejecuta los comandos especificados y devuelve propiedades de viñeta y propiedades de grupo u objeto, en lugar de la imagen representada. Consulte [Propiedades](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-response-data/c-ir-properties.md#concept-e99f1a373eae4af9b41842ca0088ad3a) para obtener una descripción de la sintaxis de respuesta y el tipo MIME de respuesta. La selección predeterminada se aplica a menos que `obj=` o `sel=` se especifique también (consulte [ `obj=` ](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-obj.md#reference-31e7dac7931b4e0eb3c7589f120a1e6a)).
...
<table id="table_F3ECF0584F6247A2A75C1CAFE1C57A4E"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p> Calidad JPEG y indicador de croma (consulte <span class="codeph"> qlt= <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-qlt.md#reference-27b91c226eb241d0a14a29af3b3afdbd" type="reference" format="dita" scope="local"> </a> </span>) </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> image.type </span> </p> </td> 
   <td> <p> Cadena </p> </td> 
   <td> <p> Tipo de MIME para la imagen de respuesta (consulte <span class="codeph"> fmt= <a href="../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-fmt.md#reference-4c743f67d56b47c5b774fcc900ff758c" type="reference" format="dita" scope="local"> </a> </span>). </p> </td> 
  </tr> 
  <tr> 
...
Devuelve el contenido de `vignette::UserData`. El servidor reemplazará todas las ocurrencias de `'??'` en `vignette::UserData` por caracteres de fin de línea ( `<cr><lf>`). La respuesta tiene el formato de datos de texto con el tipo MIME de respuesta definido como &lt;text/plain>.

Si el objeto especificado en la ruta de URL no se resuelve en una entrada de mapa de viñetas válida o si el `vignette::UserData` está vacío, la respuesta sólo contendrá un terminador de líneas ( `CR/LF`).
