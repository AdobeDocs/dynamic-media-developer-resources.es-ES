...
La imagen no se rellena si ambas `wid=` y `hei=` se especifica y `wid`/ `hei` es diferente de la relación de aspecto de la imagen.
...
Puede ocurrir en cualquier lugar dentro de la solicitud. Cambiar el tamaño de la imagen con `wid=`, `hei=`o `scl=` no cambia el valor de resolución de impresión incrustado en la imagen de respuesta. Ignorado si `scl=` se produce después de `wid=` o `hei=` en la secuencia de comandos.
