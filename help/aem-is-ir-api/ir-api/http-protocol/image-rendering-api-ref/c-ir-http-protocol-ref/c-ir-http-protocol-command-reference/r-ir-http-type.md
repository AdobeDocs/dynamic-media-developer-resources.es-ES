...
`catalog::Type` si el material está basado en una entrada de catálogo. Otherwise `type=0`. Si no se especifica, o si `type=0`se especifica, el servidor seleccionará un valor predeterminado adecuado según el objeto destinatario y los demás atributos materiales.
