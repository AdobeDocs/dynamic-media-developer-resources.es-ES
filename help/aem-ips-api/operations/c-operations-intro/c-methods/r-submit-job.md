...
Este ejemplo de código envía un trabajo de publicación de servicio de imágenes a IPS y devuelve un identificador de trabajo. Elija un solo tipo de trabajo en la solicitud. Como `userHandle` se omitió, se envían notificaciones por correo electrónico al usuario que envió el trabajo. Este trabajo de ejemplo se ejecuta inmediatamente porque `execTime` se han omitido `execSchedule` .
