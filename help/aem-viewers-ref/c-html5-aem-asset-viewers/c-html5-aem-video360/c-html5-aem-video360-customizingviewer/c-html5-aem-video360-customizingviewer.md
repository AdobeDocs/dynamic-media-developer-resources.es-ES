...
Cuando cree CSS personalizado, tenga en cuenta que el visor asigna la clase `.s7video360viewer` a su elemento DOM contenedor. Si está usando un archivo CSS externo pasado con el comando `style=`, use la clase `.s7video360viewer` como clase principal en el selector descendiente para las reglas CSS. Si está utilizando estilos incrustados en la página web, clasifique también este selector con un ID del elemento DOM contenedor de la siguiente manera:
