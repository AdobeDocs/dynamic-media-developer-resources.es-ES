...
Al crear CSS personalizada, tenga en cuenta que el visor asigna `.s7video360viewer` clase a su elemento DOM de contenedor. Si utiliza un archivo CSS externo que se pasa con `style=` un comando, utilice `.s7video360viewer` la clase como clase principal en el selector de descendientes para las reglas CSS. Si está realizando estilos incrustados en la página web, califique además este selector con un ID del elemento DOM de contenedor de la siguiente manera:
