...
Al crear CSS personalizado, tenga en cuenta que el visor asigna la clase `.s7interactiveimage` a su elemento DOM contenedor. Si está usando un archivo CSS externo pasado con el comando `style=`, use la clase `.s7interactiveimage` como clase principal en el selector descendiente para las reglas CSS. Si está agregando estilos incrustados en la página web, también debe clasificar este selector con un ID del elemento DOM contenedor de la siguiente manera:
