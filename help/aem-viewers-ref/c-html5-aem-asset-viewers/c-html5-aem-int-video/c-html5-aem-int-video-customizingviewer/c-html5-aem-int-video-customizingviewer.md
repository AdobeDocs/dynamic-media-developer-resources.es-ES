---
description: Toda la personalización visual y la mayor parte del comportamiento del visor de vídeo interactivo se realiza creando una CSS personalizada.
keywords: responsive
seo-description: Toda la personalización visual y la mayor parte del comportamiento del visor de vídeo interactivo se realiza creando una CSS personalizada.
seo-title: Personalización del visor de vídeo interactivo
solution: Experience Manager
title: Personalización del visor de vídeo interactivo
topic: Dynamic media
...
# Personalización del visor de vídeo interactivo{#customizing-interactive-video-viewer}

Toda la personalización visual y la mayor parte del comportamiento del visor de vídeo interactivo se realiza creando una CSS personalizada.
...
El visor dispone de dos archivos CSS integrados para esquemas de color &quot;claros&quot; y &quot;oscuros&quot;. La versión &quot;oscura&quot; se utiliza de forma predeterminada, pero es fácil cambiar a la versión &quot;ligera&quot; utilizando el siguiente CSS estándar:
...
Al crear CSS personalizada, tenga en cuenta que el visor asigna `.s7interactivevideoviewer` clase a su elemento DOM de contenedor. Si utiliza un archivo CSS externo que se pasa con `style=` un comando, utilice `.s7interactivevideoviewer` la clase como clase principal en el selector de descendientes para las reglas CSS. Si está realizando estilos incrustados en la página web, califique además este selector con un ID del elemento DOM de contenedor de la siguiente manera:
...
El tercer grupo de marcadores de CSS contiene `.s7device_landscape` y `.s7device_portrait`. `.s7device_landscape` se establece si el dispositivo táctil está en orientación horizontal; `.s7device_portrait` se utiliza cuando se gira el dispositivo táctil a la orientación vertical. Estos marcadores CSS están destinados a utilizarse solo en sistemas de escritorio.
...
En este ejemplo siguiente, la barra de control de vídeo se encuentra 138 píxeles por encima de la parte inferior del visor si el dispositivo táctil está en orientación vertical y, en los demás casos, la mueve a la parte inferior del visor:
...
A continuación se muestra la documentación de referencia de elementos de la interfaz de usuario que se aplica al visor de vídeo interactivo:
