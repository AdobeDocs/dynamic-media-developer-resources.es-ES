...
Ejemplo: configure un botón de tabla de contenido que se posicione 4 píxeles desde la parte inferior y 43 píxeles desde la parte izquierda de la barra de control principal; el tamaño es de 28 x 28 píxeles y se muestra una imagen diferente para cada uno de los cuatro estados de botón diferentes:
...
>[!NOTE]
>
>El elemento de lista desplegable es compatible con el selector de `state` atributos, que se puede utilizar para aplicar diferentes apariencias a los estados de elemento seleccionado y al pasar el ratón por encima.
...
>[!NOTE]
>
>Es posible ocultar el índice de la página por completo mediante la configuración `display:none` de la clase `s7index` CSS.
...
>[!NOTE]
>
>Thumb admite el selector de `state` atributos, que se puede utilizar para aplicar diferentes apariencias a los estados `up`, `down`, `over`y `disabled` pulgar.
...
>[!NOTE]
>
>Button admite el selector de `state` atributos, que se puede utilizar para aplicar diferentes apariencias a los estados de botón `up`, `down`, `over`y `disabled` .
