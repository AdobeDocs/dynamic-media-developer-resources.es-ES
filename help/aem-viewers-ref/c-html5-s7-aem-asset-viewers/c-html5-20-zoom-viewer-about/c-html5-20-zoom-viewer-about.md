...
<table id="table_ED747CC7178448919C34A4FCD18922D0"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> Se desplaza por la lista de muestras en la barra de muestras. </p> <p> Si la imagen está en estado de restablecimiento y el parámetro <span class="codeph"> frametransition </span> se establece en slide, el recurso se cambia con la animación de la diapositiva. Para otros modos <span class="codeph"> de transición de marcos </span>, el gesto realiza el desplazamiento de página nativo. </p> <p> Si la imagen se amplía, la mueve horizontalmente. Si la imagen se mueve al borde de la vista y se realiza un barrido en la misma dirección, el gesto realiza un desplazamiento de página nativo. </p> </td> 
  </tr> 
  <tr> 
...
Se recomienda utilizar una página HTML lista para usar en el modo de operación emergente. La página HTML lista para usar se denomina `ZoomViewer.html` y se encuentra en la subcarpeta `html5/` de la implementación estándar de los visores de IS, como se muestra a continuación:

`<s7viewers_root>/html5/ZoomViewer.html`

Aplique CSS personalizada para conseguir una apariencia personalizada para la página.
...
1. Añadir el archivo JavaScript del visor en la página web.
   ...
   La creación de un visor requiere que agregue una etiqueta de script en el encabezado HTML. Antes de usar la API de visor, asegúrese de incluir [!DNL ZoomViewer.js]. El archivo [!DNL ZoomViewer.js] se encuentra en la subcarpeta [!DNL html5/js/] de la implementación estándar de IS-Viewers:
...
1. Configuración del tamaño del visor.
   ...
   Para que las dimensiones de vista principales sean estáticas, defina el tamaño del visor en unidades absolutas para el componente `Container` SDK interno mediante el selector de CSS `.s7zoomviewer` `.s7container` o mediante el modificador `stagesize`.
...
1. Creación e inicialización del visor.
   ...
   A continuación se muestra un ejemplo de creación de una instancia de visor, pasando las opciones de configuración mínimas necesarias al constructor y llamando al método `init()`. En este ejemplo se supone que `zoomViewer` es la instancia del visor, `s7viewer` es el nombre del marcador de posición DIV, `http://s7d1.scene7.com/is/image/` es la dirección URL del servicio de imágenes y `Scene7SharedAssets/ImageSet-Views-Sample` es el recurso.
