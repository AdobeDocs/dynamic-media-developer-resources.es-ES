...
Cuando cree CSS personalizado, recuerde que el visor asigna la clase `.s7videoviewer` a su elemento DOM contenedor. Si usa un archivo CSS externo que se pasa con el comando `style=`, use la clase `.s7videoviewer` como clase principal en el selector descendiente para las reglas CSS. Si incrusta estilos en la página web, también clasifique este selector con un ID del elemento DOM contenedor de la siguiente manera:
