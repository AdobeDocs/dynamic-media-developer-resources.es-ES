...
Al crear CSS personalizada, recuerde que el visor asigna `.s7videoviewer` clase a su elemento DOM de contenedor. Si utiliza un archivo CSS externo que se pasa con el `style=` comando, utilice `.s7videoviewer` la clase como clase principal en el selector de descendientes para las reglas CSS. Si incrusta estilos en la página web, califique además este selector con un ID del elemento DOM de contenedor de la siguiente manera:
