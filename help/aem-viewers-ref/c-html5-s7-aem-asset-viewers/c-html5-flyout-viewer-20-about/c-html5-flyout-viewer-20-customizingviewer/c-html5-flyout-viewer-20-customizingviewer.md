...
Al crear CSS personalizada, tenga en cuenta que el visor asigna la clase `.s7flyoutviewer` a su elemento DOM contenedor. Si está utilizando un archivo CSS externo pasado con el comando `style=`, utilice la clase `.s7flyoutviewer` como clase principal en el selector de descendientes para sus reglas CSS. Si está realizando estilos incrustados en la página web, califique también este selector con un ID del elemento DOM de contenedor de la siguiente manera:
...
El uso de marcadores CSS es la forma más flexible de crear CSS adaptable diseñada que le permite dirigirse no solo al tamaño de la pantalla del dispositivo, sino también al tamaño real del visor, lo que puede resultar útil para los diseños de página web adaptables.
