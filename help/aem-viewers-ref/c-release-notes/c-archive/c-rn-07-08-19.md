...
* Se ha añadido compatibilidad con la integración con la extensión de visores Dynamic Media de Launch de Adobe.
* Se ha agregado compatibilidad con el modo de operación VR en el Oculus Browser for Video360Viewer.
* Se ha agregado el evento `RolloverKeyEvent.HREF_ACTIVATED` para permitir que el código de cliente responda a la activación del mapa de imagen en eCatalogViewer y eCatalogSearchViewer.
...
* Se ha resuelto un problema con la vista de pantalla completa en el que la página se desplazaba por los dispositivos Apple (que se ejecutan en iOS 12 y posteriores).
